...
3. На каждом эмуляторе:
   ...
   - ставится debug-сборка поверх имеющейся (`adb install -r -g`), разрешения на геолокацию и уведомления выдаются сразу;
...
| `INSTALL_FAILED_UPDATE_INCOMPATIBLE` при установке | на эмуляторе сборка, подписанная другим ключом (release или debug с другой машины) | `adb -s emulator-5554 uninstall app.hovanki` и запустить снова |
