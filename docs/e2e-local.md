...
| `e2e.location` | — | `LAT,LON`: играть в этой точке. Без него игра там, где первый эмулятор ([подробнее](e2e.md#где-идёт-игра-и-какие-здания)) |
| `e2e.buildings` | `overpass` | здания для правила: `overpass` — настоящие из OpenStreetMap вокруг игры, `fake` — тестовый квартал, `off` — без правила |

**Где будет игра.** Там, где стоит первый эмулятор: Extended Controls (`⋯` на панели эмулятора) → Location → найдите место → Set location. Сценарий создаст игру в этой точке и расставит всех вокруг неё, на скриншотах карты будут здания этого района. Или задайте точку явно: `-Pe2e.location=52.2297,21.0122`.
...
4. Сервер из jar стартует с профилем `e2e` на `:8080`, его лог — `e2e/build/reports/devices/logs/server.log`. Здания зоны он берёт из Overpass (`e2e.buildings`), нужен интернет.
5. Сервер прогревается короткой игрой ботов, потом идут сценарии. Хост создаёт игру по своей геопозиции; дальше оркестратор раз в секунду двигает геопозицию эмуляторов вокруг центра зоны (`emu geo fix`). Maestro тапает по экранам, боты играют через API, наблюдатель проверяет правду сервера. В каждой фазе — скриншоты.
...
- геопозиция — последняя точка сценария, в паре сотен метров от той, что была до прогона;
