...
3. Проверить с телефона: открыть в браузере `https://<слова>.trycloudflare.com/actuator/health` — ответ `{"status":"UP",...}`. Первые секунды после запуска новый адрес может ещё не открываться.
...
iOS job идёт 15–25 минут на каждый push в `main`, а macOS-минуты приватного репозитория тарифицируются с множителем. Если это дорого — оставить ему только ручной запуск: `if: github.event_name == 'workflow_dispatch'` у job'а `ios`.
