...
- Если у хоста нет геопозиции (свежий симулятор), приложение через 20 с говорит «не удалось определить местоположение». Тогда сценарий ставит запасную точку (парк в Киеве) и нажимает «Создать» ещё раз, с «⚠» в отчёте.
