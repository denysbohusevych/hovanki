...
Пошагово, с подготовкой, параметрами и частыми проблемами, — [e2e-local.md](e2e-local.md). Коротко:

1. Поставьте Maestro и запустите один или два эмулятора в Device Manager (Google APIs / Google Play).
2. Конфигурация из папки **E2E** (`.run/`): `E2E emulators: full-round`, `restart` или `all scenarios` → Run. Из терминала — `./gradlew :e2e:devices -Pe2e.scenario=all`.
3. Задача соберёт jar сервера и debug APK, поставит приложение на все запущенные эмуляторы, поднимет сервер с профилем `e2e` на `:8080`, прогонит сценарии и вернёт настройки эмуляторов. Отчёт — [как у скрипта](#отчёт).

Скрипт и bash не нужны: работает на macOS, Linux и Windows. iOS-симуляторы — только скриптом.
