...
- когда приложение игрока загрузило здания для карты;
...
| | Standing in an arch | Прячущийся стоит в арке сквозь квартал (4 м, по 20 м здания с боков) дольше времени раскрытия — ни предупреждения, ни раскрытия |
| | No building data | Источник не дал зданий: снапшот у всех `UNAVAILABLE`, приложения ничего не грузят, прячущегося «в квартале» не предупреждают и не раскрывают |
| | Caught inside a building | Раскрытого в здании ищущий догоняет внутри и ловит по коду, как обычно; пойманного больше не предупреждают и не показывают, игра идёт дальше |
...
|---|---|
| `full-round` | Хост создаёт игру на телефоне там, где он есть (с настройками по умолчанию, кроме времени пряток), телефоны входят по коду, боты — через API, хост выбирает ищущего. На каждом устройстве экраны сменяются по фазам: лобби → прятки → поиск → результаты. Свёрнутое приложение продолжает слать точки (foreground service / фоновый режим location), сервер видит движение. Карта: приложение каждого телефона загрузило здания зоны (сервер это видит), экран прокручивается до строки атрибуции OpenStreetMap, скриншот `map` — на нём запретные здания красным. Здания: прячущийся-телефон идёт в настоящее здание рядом, достаточно глубокое для его точности GPS; сервер уверен, что он внутри, приложение предупреждает с обратным отсчётом (скриншот); игрок остаётся, пока ищущие его не увидят (`INSIDE_BUILDING`, не выбывает, баннер без отсчёта, скриншот карты у ищущего); выходит на открытое место — раскрытие и баннер сняты. Ищущий на телефоне ловит ботов, вводя 4 цифры вручную, и телефоны: код с экрана прячущегося сверяется с TOTP сервера и принимается. Скриншоты в каждой фазе |
...
С одним устройством (как iOS в CI) проверки «код с экрана прячущегося-телефона» и «прячущийся-телефон в здании» не выполняются: прячутся только боты. Их покрывает Android с двумя эмуляторами.

### Где идёт игра и какие здания

- **Место — где устройство-хост.** Хост создаёт игру по своей текущей геопозиции, как игрок: на эмуляторе это точка из Extended Controls → Location, на симуляторе — Features → Location. Сценарий берёт центр зоны у сервера и от него расставляет остальные телефоны, ботов и места пряток. До этого момента оркестратор геопозицию хоста не трогает.
- `run-devices.sh --device-location LAT,LON` ставит устройствам их «собственную» точку до прогона — как Extended Controls. В CI это центр Варшавы (Plac Defilad, рядом с Дворцом культуры; input `device_location` у Nightly): у свежего эмулятора в CI точка в сельской Калифорнии, где зданий нет, у свежего симулятора её нет вовсе.
- `--location LAT,LON` (`-Pe2e.location=…`) — играть в заданной точке: оркестратор сразу ставит её всем устройствам.
- Если у хоста нет геопозиции (свежий симулятор, как iOS в CI), приложение через 20 с говорит «не удалось определить местоположение». Флоу создания сразу падает, сценарий ставит запасную точку (парк в Киеве) и нажимает «Создать» ещё раз, с «⚠» в отчёте.
- Место игры в отчёте — с точностью до километра («around 37.42, -122.08»): видно, какой район на карте, но не точка, где стоит эмулятор.
- **Здания — настоящие**, из OpenStreetMap через Overpass (`--buildings overpass`, по умолчанию). Места пряток сценарий выбирает на открытом месте, не ближе 12 м к стенам (`BuildingSearch`, та же `BuildingMap`, по которой судит сервер). Для проверки правила он берёт ближайшую к прячущемуся точку внутри здания глубже `accuracy + 5 м`.
- `--buildings fake` — тестовый квартал вместо Overpass; `--buildings off` — без правила.
- Overpass не ответил — игра идёт без правила. Сценарий проверяет баннер «правило зданий выключено» в лобби на каждом телефоне, а проверку правила пропускает с «⚠». Нет здания нужной глубины рядом — тоже «⚠» и пропуск.

### Как это устроено

- **UI** — Maestro-флоу в `e2e/maestro/`: `create-game`, `create-game-again`, `join-game`, `allow-location`, `start-game`, `claim-catch`, `enter-code`, `await-visible`, `await-gone`, `scroll-edge-down`, `scroll-edge-up`.
